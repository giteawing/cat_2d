...
export const level07 = {
  ...
  name: 'Квантовая лаборатория', subtitle: 'Электрополя и режим квантового туннелирования',
  theme: 'lab',
  map: buildMap(),
  start: [3, 30],
  exit: [124, 28],
  ...
  setup(k, g) {
    // A
    k.message(1, 20, 12, 10, 'Квантовая лаборатория', 'Электрополя непроходимы… почти');
    ...
    k.sign(12, 27, 'Режим квантового туннелирования: Q. Разбегись (Shift) и беги в поле — шанс 25%. Не вышло? Отскочишь. Пробуй ещё!');
    ...
    // B
    k.sign(24, 27, 'Ящик поле не пропустит — отскочит. Плита за полем. Портал в потолок НАД плитой (стреляй сквозь поле) + портал в пол здесь. Урони ящик в пол!');
    ...
    // C
    k.sign(47, 27, 'Наверху — пол из электрополя. Падать на него безопасно: оно пружинит! Но… в квантовом режиме можно и провалиться.');
    k.gift(62, 12, 'big', 'g2');
    k.sign(56, 15, 'Лестница ведёт на карниз. Спрыгни с него на поле — и взлетишь к подарку.');
    k.gift(61, 28, 'secret', 'g3');
    k.secret(59, 24, 7, 6, 's1');
    k.sign(62, 27, 'Тайная комната под полем! Лестница справа выведет наверх.');
    k.clutter(['ball', 'yarn', 'teddy', 'toyCube'], 60, 30, 5, 83);
    k.clutter(['book', 'cup', 'plant', 'lamp'], 68, 18, 10, 84);
    k.funRoom(70, 30, 10, 85, 0.7);
    // D
    k.sign(78, 27, 'Шахта с вентилятором. Сверху — потолок из поля. Включи квантовый режим: вентилятор будет бросать тебя в поле, пока не пройдёшь.');
    k.lever(81, 30, 'fanD');
    ...
    // E
    k.sign(103, 27, 'Физическая площадка: брось мяч в поле — он отскочит. А тебе — последнее поле и выход.');
